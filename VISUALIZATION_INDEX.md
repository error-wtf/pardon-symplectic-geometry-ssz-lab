...
The SSZ documentation audit is kept as data/text (`data/ssz_doc_index.json`, `docs/ssz-complete-documentation-index-audit.md`) rather than as a visualization, because the rendered chart did not add enough explanatory value.
...
| `outputs/hamiltonian_drift_report.gif` | `outputs/hamiltonian_drift_report.png` | Hamiltonian drift QA for a toy SSZ effective potential. |
| `outputs/test_validation_matrix.gif` | `outputs/test_validation_matrix.png` | Executed test coverage across invariants, numerics, guardrails and traceability. |
...
Hamiltonian drift QA for a toy SSZ effective potential.
...
Actual `unittest` results grouped by what they validate. Green means internal
implementation consistency only; it is not external mathematical or physical
validation.
